package com.example.newsfeedapp.ui.theme

import androidx.compose.ui.graphics.Color

val DarkBlack = Color(0xFF1C1E21) //Dark Background

val DarkPrimary = Color(0xFFA0A3BD) //Primary
val LightPrimary = Color(0xFF464958)

val DarkAction = Color(0xFF18848E) //Action
val LightAction = Color(0xFF2196F3)

val DarkRed = Color(0xFFC30052) //Dark Error
val LightRed = Color(0xFFFF84B7)

val LightBlack = Color(0xFF3A3B3C) //Dark Surface


val BlueGray = Color(0xFFA0A3BD)
val WhiteGray = Color(0xFFB0B3B8)